...
// taken from https://github.com/dotty-staging/dotty/blob/f80a8ddfa1afb5a4a81f33e8297f8aac068161ac/tests/run/tyql.scala
...
object Expr:
  ...
  case class Join[A <: AnyNamedTuple](a: A) extends Expr[NamedTuple.Map[A, StripExpr]]
...
/** The type of database queries. So far, we have queries that represent whole DB tables and queries that reify
...
@main def Test =
  ...
  val q5 =
    ...
    yield (name = city.name, num = addr.number)
